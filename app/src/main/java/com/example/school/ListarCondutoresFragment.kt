...
class ListarCondutoresFragment : Fragment() {
    ...
    private fun setupRecyclerView() {
        condutorAdapter = CondutorAdapter(
            onEdit = { condutorId ->
//                val action = ListarCondutoresFragmentDirections.action_condutorOptionsFragment_to_cadastrarCondutoresFragment(condutorId)
//                findNavController().navigate(action)
            ...
            onDelete = { condutor ->
                lifecycleScope.launch {
                    ...
                    val condutores = db.condutorDao().getAllCondutores()
                    condutores.collect { c -> condutorAdapter.submitList(c) }
